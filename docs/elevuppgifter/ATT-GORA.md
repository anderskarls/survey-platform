...
- [ ] **Hela momentet (elevvyer) - PLAN KLAR 2026-05-31.** Momentväg (tidslinje) + uppgiftsflöde för eleven, från designhandoff i `C:\Brain\design_handoff_hela_momentet`. Jämförelse + 6-fasplan i `05-hela-momentet-implementation.md`. Kärnfynd: designens `Module/Lesson`-tabeller ska INTE byggas - `Unit`+`lessons`-JSON räcker. Låsta beslut: StudentSidebar (designtroget), datum redigerbara i admin (nytt gränssnitt), Variant B/A som default. Enda migrationen: `Unit.period`/`goals` (kräver maskin utan 5432-block).
  ...
  - **Fas 6 (polish) KLAR 2026-05-31, EJ committad.** Ny `src/lib/moment-scoring.ts` (`quizResult`->"8/8", `draftProgress`->"11/14"); `buildMomentState` fick genomsläpp `result?`/`progress?`; båda elevsidorna hämtar nu answers.isCorrect (senaste resp. vinner) + draft-answers, visar exakt resultat på klara + "Pågår 11/14" på aktiva. tsc+eslint grönt, 15/15 assertions (inkl. regression). **HELA MOMENTET-FEATUREN (Fas 1-6) KODKLAR.** Fas 1-6 deployade (981afc6, f6d9b68, f416b1f, 46b67ba, f06fd56).
  - **Fas 1b (admin lektionsdatum) KLAR 2026-05-31, EJ committad.** API `PATCH /api/courses/[courseId]/units/[unitId]` (requireAdmin, validerar period/goals/lessons), admin momentlista + editor-sida + klientformulär `UnitEditor.tsx` (period, mål-lista, per-lektion datum+vecka), "Moment"-länk i CourseSidebar. Nu kan läraren sätta datum -> elevvyns missed/upcoming/veckogruppering aktiveras. tsc+eslint grönt. **HELA PLANEN (Fas 1-6 + 1b) KODKLAR.** Inget kvar i ursprungsplanen.
